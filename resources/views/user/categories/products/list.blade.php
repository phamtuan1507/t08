...
@section('content')
    <div class="container mx-auto p-6">
        ...
        <div class="bg-white shadow-md rounded-lg p-4 mb-6">
            <form action="{{ route('category.products.list', $category) }}" method="GET" id="filter-form">
                <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                    <!-- Lọc danh mục (ẩn vì đã theo category cụ thể) -->
                    <div class="w-full md:w-1/4 hidden">
        ...
        <!-- Danh sách sản phẩm -->
        @if($products->isEmpty())
            <p class="text-gray-600 text-center">Không tìm thấy sản phẩm nào trong danh mục {{ $category->name }}.</p>
        @else
                ...
                .then(data => {
                    if (data.length > 0) {
                        suggestionsDiv.innerHTML = data.map(product => `
                            <div class="p-2 hover:bg-gray-100 cursor-pointer" onclick="window.location.href='/category/{{ $category->slug }}/products/${product.id}'">
